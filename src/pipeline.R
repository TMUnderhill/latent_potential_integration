...
# construct lookup table: GRanges range <-> corresponding TSS
# wstssranges contains TSS site +/- 200bp range
...
# alternatively, retain genes that got NA in RNA (and replace with 0 later)
rna_atac_summ <- rna_atac_summ[!is.na(rowSums(rna_atac_summ)), ] 
# if we want to treat RNA NAs as zeros.
# rna_atac_summ <- rna_atac_summ[!is.na(rowSums(rna_atac_summ[, grepl("atac", names(rna_atac_summ))])), ] 
# rna_atac_summ[is.na(rna_atac_summ)] <- 0 
